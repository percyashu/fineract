...
public class CashierTransactionDataValidator {
    ...
    public void validateCashierAllowedDateAndTime(final Cashier cashier, final Teller teller) {
        ...
        final LocalDate fromDate = ZonedDateTime.ofInstant(cashier.getStartDate().toInstant(), ZoneId.systemDefault()).toLocalDate();
        final LocalDate endDate = ZonedDateTime.ofInstant(cashier.getEndDate().toInstant(), ZoneId.systemDefault()).toLocalDate();
    ...
    public void validateOnLoanDisbursal(AppUser user, String currencyCode, BigDecimal transactionAmount) {
        ...
        if (user.getStaff() != null) {
            String sql = "select c.id from m_cashiers c where c.staff_id = " + user.getStaff().getId() + " AND "
                    ...
                    + localDateTime.atZone(ZoneId.systemDefault()) + "') BETWEEN TIME(c.start_time) AND TIME(c.end_time)  ) end)";
            try {
